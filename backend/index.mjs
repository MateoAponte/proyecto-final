...
// const url = allData.map((item) => item.url);

// const data = await Promise.all(
//   ...[url.map(async (item) => fetch(item).then((res) => res.json()))],
// );

// const dataParsed = data.flatMap((item) => {
//   // IMPORTANT: Pokemon Script Builder
//   const abilities = item.abilities.map((item) => item.ability.name);

//   const stats = item.stats.map((item) => ({
...
//   const moves = item.moves.map((item) => item.move.name);

//   // IMPORTANT: Abilities Script Builder
//   // const effect = !!item.effect_entries.length
//   //   ? item.effect_entries
//   //   : item.flavor_text_entries;

//   // const moveDescription =
//   //   effect.find((eff) => eff.language.name === 'en') || {};
//   // const text = moveDescription.effect || moveDescription.flavor_text || '';

//   // const pokemon = item.pokemon.map((poke) => poke.pokemon.name);

//   // IMPORTANT: Ability Relation Pokemon Script Builder
//   // const relations = item.pokemon.map((poke) => ({
//   //   name: item.name,
//   //   pokemon_relation: poke.pokemon.name,
//   //   isHidden: poke.is_hidden,
//   // }));
...
const url = 'mongodb://localhost:27017/PokemonBD';
